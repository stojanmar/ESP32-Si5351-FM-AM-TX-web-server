/*To control Si5351 through esp32 webserver on local Wi Fi
 * dela na esp32 doit V1
 * var01 5.1.23 try to establish AM modulation first
 * created by stojanmar
 ...
 * ver ulp5a korigiram napako v frekvenci
 */
...
#define CORRECTION 115555 // See how to calibrate your Si5351A (0 if you do not want).
...
<center>
  ...
  <br>
   ...
   <p>Stran kreirana Oct 2023 : Stojanmar </p><br>
  </center>
</body>
<script>
...
void setup() {
  ...
  //Wire.setClock(400000);  //preklopim na višjo hitrost wire
  ...
  //si5351.set_freq(, SI5351_CLK0); // Start CLK0 (VFO)
// Set CLK0 output
  //si5351.set_freq(freq0[ch] * 100, SI5351_CLK0);
  //si5351.set_freq(frekvin * 100, SI5351_CLK0); to aktiviraj povsod da dela na 90 degrees
  si5351.set_freq(frekvin * 100, SI5351_CLK1);
  ...
  si5351.drive_strength(SI5351_CLK1, SI5351_DRIVE_4MA); // Set for max power default is 2MA  max is 8MA
...
void loop() {
  ...
  dacout = anvhod34 - 256; //sredina 9 bitno
...
// Function finding zero offsets of the ADC, not in use for now
/*void findoffset(){
